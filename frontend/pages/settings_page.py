...
with tab_general:
    ...
    new_font = st.slider(_('font_size'), 10, 24, settings.get('font_size', 14))
...
with tab_ai:
    ...
    cur_style = settings.get('ai_style', 'empathetic')
    ...
    new_nickname = st.text_input(_('ai_nickname'), value=settings.get('ai_nickname', ''))
    new_retention = st.number_input(_('history_retention'), 1, 365, settings.get('history_retention_days', 30))
    sensitivities = {'low': _('low'), 'medium': _('medium'), 'high': _('high')}
    cur_sens = settings.get('crisis_sensitivity', 'medium')
...
with tab_diary:
    ...
    cur_scale = settings.get('emotion_scale', '1-10')
    ...
    reminder_on = st.checkbox(_('daily_reminder'), value=bool(settings.get('daily_reminder_enabled', 0)))
...
if st.sidebar.button(_('back'), use_container_width=True):
    st.switch_page('streamlit_app.py' if os.path.exists('streamlit_app.py') else 'app.py')
